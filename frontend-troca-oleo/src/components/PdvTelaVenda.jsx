...
export default function PdvTelaVenda() {
  ...
  // Adiciona o produto na lista local (carrinho)
  ...
  // Envia os dados para o Spring Boot emitir a NFC-e
  const handleFinalizarEEmitirNfce = async () => {
    ...
    try {
      ...
      if (resultado.status === 'AUTORIZADO') {
        ...
        setCarrinho([]); // Limpa a venda após o sucesso
      } else {
    ...
    <div className="p-6 max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="bg-white p-6 rounded-xl shadow-md border border-slate-200 lg:col-span-1">
        ...
        <form onSubmit={handleAdicionarItem} className="space-y-4">
          ...
          >
            <Plus size={18} /> Adicionar ao Caixa
          </button>
        </form>
      </div>

      {/* LISTA DO CARRINHO E BOTÃO DE EMISSÃO FISCAL */}
      <div className="bg-white p-6 rounded-xl shadow-md border border-slate-200 lg:col-span-2 flex flex-col justify-between">
        ...
        {/* RODAPÉ E BOTÃO DE FINALIZAR & EMITIR NFC-E */}
